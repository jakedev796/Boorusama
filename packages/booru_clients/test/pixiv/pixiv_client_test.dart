...
void main() {
  group('PixivClient requests', () {
    ...
    test('parses followed illusts and reports hasMore from next_url', () async {
      server.responseBody = '{"illusts": [{"id": 1}], "next_url": "https://app-api.pixiv.net/next"}';
      final client = PixivClient(dio: dio, accessToken: 'token');
    ...
    test('parses the recommended feed, ignoring ranking_illusts, and reports hasMore from next_url', () async {
      server.responseBody = '''
        {
          "illusts": [{"id": 1}, {"id": 2}],
    ...
    final shortPageHasMoreCases = [
      (
        body: '{"illusts": [{"id": 1, "visible": false}], "next_url": "https://app-api.pixiv.net/next"}',
        expected: true,
      ),
      (
    ...
    test('never leaks the authorization header value in a failed request\'s exception', () async {
      await server.stop();
